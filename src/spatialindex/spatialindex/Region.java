...
public class Region implements IShape
{
	...
	public double getIntersectingArea(final Region r)
	{
		...
		{
			if (m_pLow[cIndex] > r.m_pHigh[cIndex] || m_pHigh[cIndex] < r.m_pLow[cIndex]) return 0.0f;
		}
	...
	{
		double mul = Math.pow(2.0, (m_pLow.length) - 1.0);
		double margin = 0.0;
	...
	public String toString(double min_lat, double max_lat, double min_lng, double max_lng)
	{
		String s = "";

		s += ((max_lat - min_lat) * m_pLow[1] + min_lat) + " " + ((max_lng - min_lng) * m_pLow[0]+ min_lng); 

		s += " : ";

		s += ((max_lat - min_lat) * m_pHigh[1] + min_lat) + " " + ((max_lng - min_lng) * m_pHigh[0]+ min_lng);
